...
With the mechanism of Subscription you can forward automaticaly selected data generated on your local instance to the cental CIRCULOOS platform. A simple registration for all data from a IEQ sensor is [available](./ramp-iiot-ld-registration-to-entities/testing/3.subscribeTo_registration_to_entities.sh) 
...
1. Verify that the L14 on the [service.sh](./../service.sh) is uncomment ie CIRCULOOS_YML="$CIRCULOOS_YML -f ramp-iiot-ld-registration-to-entities/federation.yml"
2. Update the credentials on [federation.yml](./federation.yml) with the ones that you have received on you email
3. Update the "observedAt" or "value" of any entity in [1.post-ieq-local.sh](./ramp-iiot-ld-registration-to-entities/testing/1.post-ieq-local.sh)
4. Run it  and verify that the local Orion-LD has received the data: [ramp-iiot-ld-registration-to-entities/testing/2.get-ieq-local.sh](./ramp-iiot-ld-registration-to-entities/testing/2.get-ieq-local.sh)
5. Run the subscription [3.subscribeTo_registration_to_entities.sh](./3.subscribeTo_registration_to_entities.sh) and verify that the registration was successful [4.getSubsriptions.sh](./ramp-iiot-ld-registration-to-entities/testing/4.getSubsriptions.sh)
6. Re-run the [1.post-ieq-local.sh](./ramp-iiot-ld-registration-to-entities/testing/1.post-ieq-local.sh)
